...
const createComment = async (req, res) => {
  ...
  res.status(StatusCodes.OK).json({ comments: post.comments, updated: true });
};
...
const likeComment = async (req, res) => {
  ...
  res.status(StatusCodes.CREATED).json({ post, created: true });
};
...
const updateCommentReaction = async (req, res) => {
  ...
  post = await Post.findOneAndUpdate(
    ...
    { arrayFilters: [{"idx._id": req.params.commentId}, { "x.userId": req.user._id }], new: true }
  );

  res.status(StatusCodes.CREATED).json({ post, updated: true });
};
