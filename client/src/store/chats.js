...
const slice = createSlice({
	...
	reducers: {
		listChatsRequested: (chats, action) => {
		...
		activeChatRemoved: (chats, action) => {
		...
		createChatRequested: (chats, action) => {
		...
		createChatReset: (chats, action) => {
			chats.createdChat = null;
		},
		sendMessageRequested: (chats, action) => {
		...
		sendMessageSuccess: (chats, action) => {
			...
			const idx = chats.list.findIndex(x => x._id === action.payload.conversationId);
		...
		readMessagesRequested: (chats, action) => {
			chats.loading.read = true;
			chats.error.read = null;
		},
		readMessagesSuccess: (chats, action) => {
			const { conversationId, messages } = action.payload;
			let idx = chats.list.findIndex(x => x._id === conversationId);
			chats.list[idx].messages = messages
			// chats.list[idx].messages = chats.list[idx].messages.map(msg => {
			// 	idx = messages.findIndex(x => x._id === msg._id);
			// 	if (idx !== -1) {
			// 		return {
			// 			...msg,
			// 			members: messages[idx].members
			// 		}
			// 	};
			// 	return msg;
			// });
		...
		readMessageRequested: (chats, action) => {
		...
		readMessageSuccess: (chats, action) => {
			...
			let idx = chats.list.findIndex(x => x._id === conversationId);
			let midx = chats.list[idx].messages.findIndex(x => x._id === messageId);
...
export const createNewConversation = (receiverId) => async (dispatch, getState) => {
	try {
...
export const readMessagesInConversation = (conversationId) => async (dispatch, getState) => {
	try {
		...
		dispatch(readMessagesSuccess({ messages: data.messages, conversationId }))
...
export const readMessageInConversation = (conversationId, messageId) => async (dispatch, getState) => {
	const state = getState();
	const { auth, entities: { chats } } = state;
...
export const sendMessage = ({text, activeChatId, receiverId}) => async (dispatch, getState) => {
	try {
