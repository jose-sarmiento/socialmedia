	...
	async (dispatch) => {
		...
		try {
			const { data: data1 } = await axios.post(
				`${process.env.REACT_APP_API_ENDPOINT}/users/${recipient._id}/friends`,
				{},
				{
					headers: { Authorization: `Bearer ${token}` },
				}
			...
			const { data: data2 } = await axios.post(
				`${process.env.REACT_APP_API_ENDPOINT}/notifications`,
				{
					to: {
				      	_id: recipient._id,
			...
			dispatch({ type: users.addFriendSuccess, payload: {recipient: data1, notification: data2} });
		} catch (error) {
			dispatch({
...
export const confirmRequest =
	...
	async (dispatch) => {
		...
		try {
			...
			dispatch({
				type: users.confirmFriendSuccess,
